...
void InstancedRenderer::DrawAsQuads()
{
    ...
    bindTextures(gameObjectsInstanced.numTextures,
                 gameObjectsInstanced.textures, textureIndex);
...
void InstancedRenderer::Draw()
{
    ...
    unsigned int instanceCount =
        this->gameObjectsInstanced.modelMatrices.size();

    glBindVertexArray(this->quadVAO);
    glDrawElementsInstanced(
        GL_TRIANGLES, this->gameObjectsInstanced.indices.size(),
        GL_UNSIGNED_INT, (void *)this->gameObjectsInstanced.indices[0],
        instanceCount);
    glBindVertexArray(0);
}