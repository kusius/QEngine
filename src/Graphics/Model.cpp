...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    if (mesh->mMaterialIndex >= 0)
    {
        ...
        textures.insert(textures.end(), emissionMaps.begin(),
                        emissionMaps.end());

        vector<Texture> normalMaps = loadMaterialTextures(
...
unsigned int Model::loadTextureFromFile(const char *file,
                                        const string &directory)
{
    ...
    if (!image)
    {
        cout << "LOAD_TEXTURE::FAILED: " << stbi_failure_reason() << " " << path
             << endl;
