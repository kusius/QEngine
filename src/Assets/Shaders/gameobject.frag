...
{
  vec3 position;
  // same color intensities for the light probe which illuminates our object
  vec3 direction;
  vec3 ambient;
  vec3 diffuse;
  vec3 specular;
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
  ...
  // ambient += light.ambient  * vec3(texture(material.texture_diffuse2,
  // TexCoords)); diffuse += light.diffuse  * diff *
  // vec3(texture(material.texture_diffuse2, TexCoords)); specular =
  // light.specular * spec * vec3(texture(material.texture_specular2,
  // TexCoords));
...
vec3 CalcSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
  ...
  vec3 diffuse = light.diffuse *
                 (diff * vec3(texture(material.texture_diffuse1, TexCoords)));
  diffuse *= intensity;

  // specular
  float specularStrength = 0.1;
  vec3 reflectDirection = reflect(-lightDirection, normal);
  float spec = pow(max(dot(viewDir, reflectDirection), 0.0),
                   material.shininess); // the power raised is the shininess
